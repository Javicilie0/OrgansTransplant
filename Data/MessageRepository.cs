    ...
    public class MessageRepository : IMessageRepository
    {
        ...
        public async Task SendRequestAsync(Message message)
        {
            await Task.Run(() =>
            {
                ...
                {
                    Logger.LogError("Failed to send request", ex);
        ...
        public async Task RespondToRequestAsync(int messageId, MessageStatus status, DeliveryOption deliveryOption, string responseText)
        {
            await Task.Run(() =>
            {
                ...
                {
                    Logger.LogError($"Failed to respond to request: {messageId}", ex);
        ...
        public async Task<List<Message>> GetReceivedMessagesAsync(string hospitalName)
        {
            return await Task.Run(() =>
            {
                ...
                {
                    Logger.LogError($"Failed to get received messages for: {hospitalName}", ex);
        ...
        public async Task<List<Message>> GetSentMessagesAsync(string hospitalName)
        {
            return await Task.Run(() =>
            {
                ...
                {
                    Logger.LogError($"Failed to get sent messages for: {hospitalName}", ex);
        ...
        public async Task<int> GetUnreadMessagesCountAsync(string hospitalName)
        {
            return await Task.Run(() =>
            {
                ...
                {
                    Logger.LogError($"Failed to get unread messages count for: {hospitalName}", ex);
        ...
        public async Task DeleteMessageAsync(int messageId)
        {
            await Task.Run(() =>
            {
                ...
                {
                    Logger.LogError($"Failed to delete message: {messageId}", ex);
                    throw new Exception($"Грешка при изтриване на съобщение: {ex.Message}");
                }
            });
        }

        private Message MapMessageFromReader(MySqlDataReader reader)
        {
            return new Message
            {
